...
def ModelCompile(opt, lr, loss):
    ...
    model.compile(optimizer=opt, loss=loss, metrics=['accuracy'])    #optimizer 변경 (loss는 건듦x)

def ModelFit(epc):
    print("epc: ", epc)
    model.fit(x_train, y_train, epochs=epc, verbose=0, validation_split=0.01)    #epochs변경, verbose가 0이면 깔끔하게 출력, 1이면 쪼금 자세히 1로해서내. 기존 1이고 split은 1이 가장 best
...
optimizer = ['sgd'] #xiver 있다던데 'sgd', 'adam', 'rmsprop', 'adadelta','adagrad'
loss = ['mse'] #, 'mse', 'binary_crossentropy', 'categorical_crossentropy']  #'BinaryCrossentropy class', 'CategoricalCrossentropy class', 'SparseCategoricalCrossentropy class', 'Poisson class', 'binary_crossentropy function', 'categorical_crossentropy function', 'sparse_categorical_crossentropy function', 'poisson function', 'KLDivergence class', 'kl_divergence function'
lr = [0.1, 0.01]
epc = [1, 10]
